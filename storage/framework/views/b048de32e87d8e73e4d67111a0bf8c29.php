...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <form method="POST" action="<?php echo e(route('admin.market-prices.update')); ?>" class="grid grid-cols-1 md:grid-cols-5 gap-4">
            <?php echo csrf_field(); ?>
                ...
                <tbody class="divide-y divide-gray-200">
                    <?php $__empty_1 = true; $__currentLoopData = $prices; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $price): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 font-semibold text-gray-900"><?php echo e($price->commodity); ?></td>
                            <td class="px-6 py-4 text-gray-900">PHP <?php echo e(number_format((float) $price->price_per_kilo, 2)); ?></td>
                            <td class="px-6 py-4 text-gray-600"><?php echo e($price->date_updated?->format('M d, Y')); ?></td>
                            <td class="px-6 py-4 text-gray-600"><?php echo e($price->source_market ?? 'N/A'); ?></td>
                            <td class="px-6 py-4">
                                <div class="flex justify-center gap-2">
                                    <form method="POST" action="<?php echo e(route('admin.market-prices.row.update', $price)); ?>" class="flex gap-1">
                                        <?php echo csrf_field(); ?>
                                        <?php echo method_field('PUT'); ?>
                                        <input type="hidden" name="commodity" value="<?php echo e($price->commodity); ?>">
                                        <input type="hidden" name="price_per_kilo" value="<?php echo e($price->price_per_kilo); ?>">
                                        <input type="hidden" name="date_updated" value="<?php echo e(optional($price->date_updated)->format('Y-m-d')); ?>">
                                        <input type="hidden" name="source_market" value="<?php echo e($price->source_market); ?>">
                                        <button class="px-3 py-1 text-xs bg-blue-600 text-white rounded">Keep</button>
                                    </form>
                                    <form method="POST" action="<?php echo e(route('admin.market-prices.row.destroy', $price)); ?>" onsubmit="return confirm('Delete this price row?')">
                                        <?php echo csrf_field(); ?>
...
<?php /**PATH C:\Users\jayso\Desktop\dev\web\Mannalon-App\resources\views/admin/market-prices-content.blade.php ENDPATH**/ ?>