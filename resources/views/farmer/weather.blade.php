...
@section('page-title', 'Weather Information')
@section('page-subtitle', '7-day forecast and farming advisories')
...
<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-white to-gray-50 rounded-xl shadow-lg p-8 text-gray-800 border-l-4 border-emerald-600">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            ...
            <div class="text-center">
                ...
                <p class="text-gray-600">Last updated: {{ date('F d, Y h:i A') }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-md p-6">
        ...
        <div class="space-y-3">
            @forelse($weatherAnnouncements as $announcement)
                @php
                ...
                @endphp
                <div class="{{ $boxClass }} border-l-4 p-4 rounded-r-lg">
                    <div class="flex items-start gap-3">
                        <i class="{{ $iconClass }} text-xl mt-1"></i>
                        <div>
                            <h4 class="font-semibold {{ $titleClass }}">{{ $announcement->title }}</h4>
                            <p class="text-sm {{ $textClass }} whitespace-pre-line">{{ $announcement->content }}</p>
                            <p class="text-xs mt-2 {{ $textClass }}">Posted {{ $announcement->created_at->format('F d, Y') }}</p>
                        </div>
                    </div>
                </div>
            @empty
    ...
    <div class="bg-white rounded-xl shadow-md p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-7 gap-4">
            ...
            <div class="bg-gray-50 rounded-lg p-4 text-center hover:bg-gray-100 transition border border-gray-200">
                <p class="font-semibold text-gray-800 mb-1">{{ $day['day'] }}</p>
                <p class="text-xs text-gray-500 mb-3">{{ $day['date'] }}</p>
                <div class="text-5xl mb-3">{{ $day['icon'] }}</div>
                <p class="text-sm font-semibold text-gray-700 mb-2">{{ $day['desc'] }}</p>
                <div class="flex justify-center gap-2 text-sm mb-2">
                    <span class="text-red-600 font-bold">{{ $day['high'] }}°</span>
                    <span class="text-gray-400">/</span>
                    <span class="text-blue-600 font-bold">{{ $day['low'] }}°</span>
                </div>
                <p class="text-xs text-blue-600">💧 {{ $day['rain'] }}% rain</p>
            </div>
            @endforeach
        </div>
    </div>
