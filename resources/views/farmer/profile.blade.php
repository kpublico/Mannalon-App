...
@section('page-subtitle', 'View your personal information and change password')
...
<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-emerald-600 to-emerald-800 rounded-xl p-8 text-white">
        <div class="flex items-center gap-6">
            ...
            <div>
                <h2 class="text-3xl font-bold mb-2">{{ auth()->user()->name ?? 'Farmer' }}</h2>
                <p class="text-emerald-100 mb-1">📧 {{ auth()->user()->email }}</p>
                <p class="text-emerald-100">👤 {{ ucfirst(auth()->user()->role ?? 'farmer') }} - Viewer Access</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-md p-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <div class="px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg text-gray-800">
                    {{ auth()->user()->barangay ?? 'Not provided' }}
                </div>
            </div>
            ...
            <div>
                ...
                <div class="px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg text-gray-800">
                    {{ auth()->user()->zone_purok ?? 'Not provided' }}
                </div>
            </div>
            ...
            <div>
                ...
                <div class="px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg text-gray-800">
                    {{ auth()->user()->house_number ?? 'Not provided' }}
                </div>
            </div>
                ...
                <div class="px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg">
                    ...
                    @else
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold bg-gray-100 text-gray-800">
                            {{ ucfirst(auth()->user()->status ?? 'pending') }}
                        </span>
                    @endif
                </div>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-md p-8">
        ...
        <form action="{{ route('farmer.profile.password') }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
    ...
    <div class="bg-gray-50 rounded-xl border border-gray-200 p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            ...
            <div>
                ...
                <p class="font-semibold text-gray-800">{{ Auth::user()->updated_at->format('F d, Y') }}</p>
            </div>
            <div>
