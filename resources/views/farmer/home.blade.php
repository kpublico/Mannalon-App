...
@section('page-subtitle', 'View-only access to farm information and learning resources')

@section('content')
<div class="space-y-6">
    <section class="rounded-2xl bg-gradient-to-r from-emerald-700 via-emerald-600 to-teal-600 p-6 md:p-8 text-white shadow-lg">
        <div>
            <h2 class="text-3xl md:text-4xl font-extrabold">Welcome {{ auth()->user()->name ?? 'Farmer' }}</h2>
            ...
            <div class="mt-4 flex flex-wrap gap-2 text-xs md:text-sm">
                ...
                <span class="px-3 py-1 rounded-full bg-white/20 border border-white/30">Location: {{ auth()->user()->barangay ?? 'Barangay not set' }}</span>
                <span class="px-3 py-1 rounded-full bg-white/20 border border-white/30">{{ now()->format('F d, Y') }}</span>
            </div>
        </div>
    </section>
    ...
    <section class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 md:p-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4">
            <a href="{{ route('farmer.announcements') }}" class="group rounded-xl border border-emerald-200 bg-emerald-50 p-4 hover:shadow-md transition">
            ...
            <a href="{{ route('farmer.guides') }}" class="group rounded-xl border border-blue-200 bg-blue-50 p-4 hover:shadow-md transition">
            ...
            <a href="{{ route('farmer.weather') }}" class="group rounded-xl border border-cyan-200 bg-cyan-50 p-4 hover:shadow-md transition">
            ...
            <a href="{{ route('farmer.market-prices') }}" class="group rounded-xl border border-amber-200 bg-amber-50 p-4 hover:shadow-md transition">
            ...
            <a href="{{ route('farmer.information') }}" class="group rounded-xl border border-teal-200 bg-teal-50 p-4 hover:shadow-md transition">
            ...
            <a href="{{ route('farmer.guides') }}" class="group rounded-xl border border-fuchsia-200 bg-fuchsia-50 p-4 hover:shadow-md transition">
        ...
        <article class="bg-white rounded-xl border border-gray-200 p-5 lg:col-span-2">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div class="rounded-lg bg-gray-50 border border-gray-200 p-4">
                    ...
                    <p class="text-2xl font-extrabold text-gray-900">{{ $newNotices ?? 0 }}</p>
                    <p class="text-xs text-emerald-700">Check announcements</p>
                </div>
                <div class="rounded-lg bg-gray-50 border border-gray-200 p-4">
                    <p class="text-sm text-gray-500">Price Updates</p>
                    <p class="text-2xl font-extrabold text-gray-900">{{ $latestPriceDate ? \Carbon\Carbon::parse($latestPriceDate)->format('M d') : 'N/A' }}</p>
