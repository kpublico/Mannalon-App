...
@section('page-subtitle', 'Add a new announcement for farmers')
        ...
        <form method="POST" action="{{ route('admin.announcements.store') }}">
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" name="title" class="w-full border-2 border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:border-emerald-600" placeholder="Enter announcement title" value="{{ old('title') }}" required>
                    @error('title')<span class="text-red-600 text-sm">{{ $message }}</span>@enderror
                </div>

                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Content</label>
                    <textarea name="content" rows="5" class="w-full border-2 border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:border-emerald-600" placeholder="Enter announcement content" required>{{ old('content') }}</textarea>
                ...
                <div>
                    ...
                    <select name="category" class="w-full border-2 border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:border-emerald-600" required>
                        ...
                        <option value="Alert" {{ old('category') == 'Alert' ? 'selected' : '' }}>Alert</option>
                ...
                <div class="border-t pt-6">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="audience_scope" class="w-full border-2 border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:border-emerald-600" id="audience_scope" required>
                            ...
                            <option value="all" {{ old('audience_scope') == 'all' ? 'selected' : '' }}>All Farmers</option>
                            <option value="specific_group" {{ old('audience_scope') == 'specific_group' ? 'selected' : '' }}>Specific Farmer Group</option>
                        </select>
                        @error('audience_scope')<span class="text-red-600 text-sm">{{ $message }}</span>@enderror
                    </div>

                    <div id="target_group_container" class="mb-4" style="display: {{ old('audience_scope') == 'specific_group' ? 'block' : 'none' }}">
                        ...
                        <select name="target_group_id" class="w-full border-2 border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:border-emerald-600">
                            ...
                            @forelse($groups as $group)
                                <option value="{{ $group->id }}" {{ old('target_group_id') == $group->id ? 'selected' : '' }}>{{ $group->group_name }} ({{ $group->region }})</option>
                            @empty
                ...
                <div class="border-t pt-6">
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <input type="datetime-local" name="starts_at" class="w-full border-2 border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:border-emerald-600" value="{{ old('starts_at') }}">
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="ends_at" class="w-full border-2 border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:border-emerald-600" value="{{ old('ends_at') }}">
                    ...
                    <div>
                        ...
                        <input type="date" name="expiry_date" class="w-full border-2 border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:border-emerald-600" value="{{ old('expiry_date') }}">
